...
class TM_WC_Grid_List {
	...
	public function add_product_class( $classes, $class = '', $post_id = '' ) {
		...
		if ( $product && is_main_query() ) {
			...
			if ( 'list' === $this->condition && ( is_product_taxonomy() || is_post_type_archive( 'product' ) || is_page( wc_get_page_id( 'shop' ) ) ) ) {
	...
	public function add_toggle_button() {
		...
		$html     = array();
		$html[]   = '<div class="tm-woo-grid-list-toggle-button-wrapper">';
